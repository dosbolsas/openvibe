...
## How specs stay current (and when they don't)
Drift on a capability self-heals the next time anyone touches it: @architect reads the spec AND the actual code before planning, notices the contradiction, and directs a spec correction. @2-code-review then catches misapplied updates. This costs zero extra tokens — the read happens anyway. The residual gap is a capability nobody touches for a long time, or a direct commit that bypasses the pipeline entirely. Both are accepted (the alternative is spending review tokens every task to detect drift, which fights the token-minimal goal). If rare-capability drift ever bites in practice, the cheap fix is a periodic manual audit: skim `specs/*.md` against the code for untouched capabilities. Near-zero tokens, runs only when you choose, no per-clone setup. Don't run it preemptively — run it the first time drift actually bites, because that tells you which capabilities are worth auditing.
